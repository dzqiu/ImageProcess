...
//         cout<<" propare "<<((float)cvGetTickCount() - t)/(cvGetTickFrequency()*1e6)<<endl;
//         t = (float)cvGetTickCount();
        cvMatMul(A, VX, VX);
        cvMatMul(A, VY, VY);
//        cout<<A->cols<<"*"<<A->rows<<" = "<<((float)cvGetTickCount() - t)/(cvGetTickFrequency()*1e6)<<endl;
//        t = (float)cvGetTickCount();
    }
    for( i = 0; i < n; i++ )
    {
...
/*my function*/
CvPoint*
cvSnakeImageGVFloop(//const CvArr* srcarr,
                CvPoint* points,
                int *length,
                float alpha,
                float beta,
...
//    float t = (float)cvGetTickCount();
    ...
    {
        icvSnake32FC1_GVF( u, v, points, length, alpha, beta, gamma, kappa, ITER_int, flag);
//        cout<<"icvSnake32FC1_GVF computing time = "<<((float)cvGetTickCount() - t)/(cvGetTickFrequency()*1e6)<<endl;
//        t = (float)cvGetTickCount();
    if(flag == CV_REINITIAL){
      points = cvSnakeInterp(points, length, 1, 10, CV_WITHOUT_HUN);
//      cout<<"cvSnakeInterp computing time = "<<((float)cvGetTickCount() - t)/(cvGetTickFrequency()*1e6)<<endl;
//      t = (float)cvGetTickCount();
        }
//      loadBar(i+1, ITER_ext, 50);
    }
